...
public struct Rules: PERulesProtocol {
    
    public typealias Searcheable = Materia
    
    private var foundSubmatter: Bool = false
    private var foundProgramContentTitle: Bool = false
    private var foundProgramContentDescription: Bool = false
    
    private var foundNextMatter = false
    
    public var result: Array<Searcheable> = []
   
    private var materia: Searcheable?
    
    private var conteudoProgramatico: ConteudoProgramatico?
    
    public init() {}
    
    mutating public func predefinedValues(ofKey key: String) {
        if key == "w:b" { foundSubmatter = true }
        if key == "w:u" { foundProgramContentTitle = true }
        if key == "w:i" { foundProgramContentDescription = true }
    }
    
    mutating public func search(foundtext: String) -> String {
        if foundtext.isEmpty { return foundtext }
        if foundSubmatter {
            foundNextMatter = true
            materia = Materia()
            materia?.title = foundtext
            result.append(materia!)
        }
        if foundProgramContentTitle {
            conteudoProgramatico = ConteudoProgramatico()
            conteudoProgramatico?.title = foundtext
            materia?.conteudoProgramatico = []
            materia?.conteudoProgramatico?.append(conteudoProgramatico!)
        }
        if foundProgramContentDescription {
            let conteudo = conteudoProgramatico?.description ?? ""
            conteudoProgramatico?.description = conteudo + foundtext
        }
    ...
    mutating public func reset() {
        foundSubmatter = false
        foundProgramContentTitle = false
        foundProgramContentDescription = false
    }
    
}

public final class Materia: Codable, @unchecked Sendable {
    var title: String? = nil
    var conteudoProgramatico: [ConteudoProgramatico]? = nil
}

public final class ConteudoProgramatico: Codable, @unchecked Sendable {
    var title: String? = nil
